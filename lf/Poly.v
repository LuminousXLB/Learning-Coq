...
Proof.
  intros.
  assert (H: forall t:list X, forall h: X, map f (t ++ [h]) = map f (t) ++ [f h]). {
    intros. induction t.
    - reflexivity.
    - simpl. rewrite -> IHt. reflexivity.
  }
  induction l as [| h t].
  - reflexivity.
  - simpl. rewrite -> H. rewrite -> IHt. reflexivity.
Qed.
(** [] *)
...
Theorem fole_map_correct : forall (X Y: Type) (f: X -> Y) (l: list X),
